...
/**
 * The Java-Class AniamtionProcess starts the starts the calculation of the
 * animation and when a train can Move and start the waltTo Methode
 * 
...
public class AnimationProcess {
	...
	/**
	 * Method startAnimation starts all Animation of the Trains witch are drawn
	 * on the Map
	 * 
	 * @param p
	...
	public static void startAnimations(Project p, NodeMap map) {
		...
		for (TrainData train : p.getTraindataProjectList()) {
			map.getMobileObjects().get(String.valueOf(train.getID()))
					.startAnimation();
		}
	}
	 ...
	 *            - NodeMap with all Nodes and Paths
	 */
	public static void calculateSimulation(Project p, NodeMap map, int h,
			int min) {
		...
		for (TrainData train : p.getTraindataProjectList()) {
			...
			TrainFigure tf = (TrainFigure) map.getMobileObjects().get(
				...
				if (!tt.isDrived() && !tt.isBlocked()
					...
					if (tt.getVisits() == tt.getMiddlestations().size()) {
						...
						AnimationEvents.walkTo(
								tf,
								map.getNodes().get(
										String.valueOf(tt.getMiddlestations()
												.get(tt.getVisits()))), map);
						tt.setVisits(tt.getVisits() + 1);
	 ...
	 */
	private static void sortedTrains(ArrayList<TrainData> traindataProjectList,
		...
		while (unsortiert) {
			...
			for (int i = 0; i < traindataProjectList.size() - 1; i++) {
				...
				int second = getValue(traindataProjectList.get(i + 1)
						.getPriority());
				if (first > second) {
					tdTemp = traindataProjectList.get(i);
					traindataProjectList
							.set(i, traindataProjectList.get(i + 1));
					traindataProjectList.set(i + 1, tdTemp);

					ttTemp = timeTableProjectList.get(i);
					timeTableProjectList
							.set(i, timeTableProjectList.get(i + 1));
