...
/**
 * Class that contain different Methods to open different Dialogs for edit, add,
 * delete, import export Train/Timetable/Projects, close the Programm, show help
 * and info Dialog.
 * 
...
public class CentralEventController {
	...
	/**
	 * This method open a Dialog with a request at the User and close the
	 * program if the Yes- Button is pushed and close the dialog if the no Button
	 * is pushed.
	 * 
	 */
	 ...
	 * 
	 * @param tab
	 *            - index for the different TabFolders.
	 */
	...
	/**
	 * Open a AddDialog to add a Train or a Timetable or a Project open a
	 * ProjectAddDialog if addDialog = 2 open a TimetableAddDialog if addDialog
	 * = 1 open a TrainAddDialog id addDialog = 0
	 * 
	 * @param addDialog
	 *            - int witch Dialog should be open
	 */
		...
		if (addDialog == 0) {
			...
			if ((Main.getTimetableListAll().size() > 0)
				...
				if (Main.getTimetableListAll().size() < 1) {
					...
					message = message
							+ "Es gibt keine Fahrpläne daher kann kein Project erstellt werden. \n";

				}
				if (Main.getTrainListAll().size() < 1) {

					message = message
							+ "Es gibt keine Züge daher kann kein Project ertstellt werden.";
	...
	/**
	 * Open a EditDialog to edit a Train or a Timetable or a Project open a
	 * ProjectEditDialog if addDialog = 2 open a TimetableEditDialog if
	 * addDialog = 1 open a TrainEditDialog id addDialog = 0
	 * 
	 * @param menu
	 * @param editDialog
	 *            - int witch Dialog should be open
		...
		if (editDialog == 0) {
			...
			if (Main.getTrainListAll().size() > 0) {
				...
				MessageBox messageBox = new MessageBox(Main.getMainShell(),
						SWT.ERROR | SWT.OK);
				messageBox
						.setMessage("Es gibt keine Züge die Bearbeitet werden können.");
				messageBox.open();
			...
			if (Main.getTimetableListAll().size() > 0) {
				...
				new TimetableEditDialog(Main.getMainShell(), SWT.NONE)
						.open(menu);
				...
				MessageBox messageBox = new MessageBox(Main.getMainShell(),
						SWT.ERROR | SWT.OK);
				messageBox
						.setMessage("Es gibt keine Fahrpläne die Bearbeitet werden können.");
				messageBox.open();
	 ...
	 * 
	 * open a TimetableDeleteDialog if addDialog = 1 open a TrainDeleteDialog id
	 * addDialog = 0
	 * 
	 * @param menu
	 * @param deleteDialog
		...
		if (deleteDialog == 0) {
			...
			if (Main.getTrainListAll().size() > 0) {
				...
				MessageBox messageBox = new MessageBox(Main.getMainShell(),
						SWT.ERROR | SWT.OK);
				messageBox
						.setMessage("Es gibt keine Züge die Gelöscht werden können.");
				messageBox.open();
			...
			if (Main.getTimetableListAll().size() > 0) {
				...
				new TimetableDeletDialog(Main.getMainShell(), SWT.NONE)
						.open(menu);
				...
				MessageBox messageBox = new MessageBox(Main.getMainShell(),
						SWT.ERROR | SWT.OK);
				messageBox
						.setMessage("Es gibt keine Fahrpläne die Gelöscht werden können.");
				messageBox.open();
	 ...
	 * 
	 * open a ProjectSaveDialog if addDialog = 2 open a TimetableSaveDialog if
	 * addDialog = 1 open a TrainSaveDialog id addDialog = 0
	 * 
	 * @param menu
	 * @param saveDialog
	 */
	public static void save(boolean menu, int saveDialog) {
		...
		if (saveDialog == 0) {
			...
			if (menu == false) {
				...
				TableItem[] rowData = CompositeTrain.getTrainTable()
						.getSelection();
			...
			if (menu == false) {
				...
				String idCheck = TimetableExportDialog.comboTimetables
						.getText();
				Timetable tt = Main.getTimetableListAll().get(0);
				int i = 0;
			...
			if (menu == false) {
				...
				TableItem[] rowData = CompositeProject.getProjectTable()
						.getSelection();
	 ...
	 * 
	 * open a ProjectLoadDialog if addDialog = 2 open a TimetableLoadDialog if
	 * addDialog = 1 open a TrainLoadDialog id addDialog = 0
	 * 
	 * @param openDialog
	 */
	...
	/**
	 * This methode create and open a new export Dialog for Train/Timetable or
	 * Project.
	 * 
	 * 
	 * @param exportDialog
		...
		if (exportDialog == 0) {
			...
			if (Main.getTrainListAll().size() > 0) {
				...
				MessageBox messageBox = new MessageBox(Main.getMainShell(),
						SWT.ERROR | SWT.OK);
				messageBox
						.setMessage("Es gibt keine Züge die Exportiert werden können.");
				messageBox.open();
			...
			if (Main.getTimetableListAll().size() > 0) {
				...
				new TimetableExportDialog(Main.getMainShell(), SWT.NONE)
						.open(true);
				...
				MessageBox messageBox = new MessageBox(Main.getMainShell(),
						SWT.ERROR | SWT.OK);
				messageBox
						.setMessage("Es gibt keine Fahrpläne die Exportiert werden können.");
				messageBox.open();
			...
			if (Main.getProjectListAll().size() > 0) {
				...
				new ProjectExportDialog(Main.getMainShell(), SWT.NONE)
						.open(true);
				...
				MessageBox messageBox = new MessageBox(Main.getMainShell(),
						SWT.ERROR | SWT.OK);
				messageBox
						.setMessage("Es gibt keine Projecte die Exportiert werden können.");
				messageBox.open();
