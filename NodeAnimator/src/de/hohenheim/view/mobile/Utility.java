...
public class Utility {
	...
	/**
	 * Calculates the distance value on the path between the start room and the
	 * end room.
	 * 
	...
	/**
	 * Determines the room at the end of the path regarding the start room which
	 * we pass as parameter.
	 * 
	 * @param {@link PathFigure} path - The path where we want to have the end
	 *        room.
	 * @param {@link NodeFigure} start - The start room.
	 * @return {@link NodeFigure} room - the room which is at the other end of
	 *         the path regarding the start room.
	 */
	...
	public static boolean hasEdge(
			HashMap<NodeFigure, ArrayList<PathFigure>> paths, NodeFigure start,
			NodeFigure end) {
	...
	/**
	 * Calculates a route between two rooms. this algorithm is quite simple and
	 * does not optimize the route (no shortest paths).
	 * 
	 * @param {@link NodeMap} map - The RoomMap where the rooms are located.
	 * @param {@link NodeFigure} start - The start room where the calculation
	 *        begins.
	 * @param {@link NodeFigure} end - The end room where the calculation should
	 *        stop.
	 * @return {@link Iterator} with all RoomFigures ({@link NodeFigure}) which
	 *         mark the route.
	 */
	public static Iterator<NodeFigure> getRoute(NodeMap map, NodeFigure start,
			NodeFigure end) {
		...
		// Cut off the start_room, we don't need it. it must be saved in the
		// personfigure for the animation.
		ArrayList<NodeFigure> tmppath = getRoute(roomList, roomVisited,
				edgeVisited, map.getPaths(), start, end);
	...
	private static ArrayList<NodeFigure> getRoute(
			ArrayList<NodeFigure> roomList, ArrayList<NodeFigure> roomVisited,
				...
				if (!edgeVisited.contains(next_path)) {
					if (!roomList.contains(end)) {
						if (!roomList.contains(next_start)) {
							...
							roomList = getRoute(roomList, roomVisited,
									edgeVisited, paths, next_start, end);
						}
					}
				}
			}
		...
		// Optimize. we sometimes have nodes inside which are wrong. We now
		// filter them out...
	...
	public static Iterator<NodeFigure> getOptimizedRoute(NodeMap map,
			NodeFigure start, NodeFigure end) {
