 ...
 * @version 1.0
 * 
 *          This class represents a ForkTruck. To exist in the system a mobile
 *          object must be located in a RoomMap. This means either in a Room or
 *          on a Path between rooms.
 * 
 *          When the mobileFigure Object is instantiated, this Object must be
 *          located in a room, for example in the "entrance" of the RoomMap.
 */
public class TrainFigure extends AnimationFigure {
	...
	/**
	 * Constructor. Sets the mobile object we want to create inside a room (as
	 * starting point)
	 * 
	 * @param {@link NodeMap} map - The map where the figure should be placed.
	 * @param NodeFigure
	 *            The Room where the mobile object is located when it is
	 *            instantiated.
	 * @param int id - a unique id for the fork truck.
	 */
	public TrainFigure(NodeMap map, NodeFigure in_room, int id,
			Object modellObject) {
	...
	public void paintFigure(Graphics g) {
		...
		Font f = this.getFont();
		Dimension dim_text = FigureUtilities.getTextExtents("" + getFigureId(),
				f);
	...
	private Image getTrainImage(int id) {
		...
		for (int j = 0; j < Main.getTrainListAll().size(); j++) {
			...
			if (td.getID() == id) {
				...
				} else if (td.getTypOfTrain().equalsIgnoreCase("G¸terzug")) {
	 ...
	 * 
	 * @param State
	 *            state. The State of n object the fork truck is waiting for.
	 */
