 ...
 * current position along a path to a specified room.
 * 
...
public class WalkToAnimator extends Observable implements Runnable, Animator {
	...
	/**
	 * The room where the figure should go. this is a temporal position at a
	 * moment during the animation because along a path a figure can visit a lot
	 * of rooms.
	 * 
	 */
	NodeFigure end_node;
	...
	/**
	 * This is the room where we really want to go. So the last room in this
	 * animation.
	 */
	 ...
	 * This means: if the animation has more then one direct paths from one room
	 * to another then for each inner animation init must be set to true.
	 */
	boolean init;
	...
	/**
	 * We use a {@link ManhattanConnectionRouter}. So there can be rectangular
	 * branches along a path between two rooms. To animate the figure along this
	 * paths we separate all horizontal and vertical lines as segments to change
	 * direction (e.g. left then up).
	 */
	PointList segments = null;

	/**
	 * The run_count for each point inside a segment. So this can be from 0 to
	 * segment.size()
	 */
	int run_count = 0;
	...
	/**
	 * Indicates that a stopaction has started. Problem is that we nust wait
	 * till it is really stopped!
	 */
	boolean stop = false;
	 ...
	 */
	public WalkToAnimator(NodeMap map, AnimationFigure figure,
			NodeFigure end_node) {
	...
	/**
	 * Helper method to get a path from the figures ({@link AnimationFigure})
	 * current position to the specified node.
	 * 
	 * @param {@link NodeFigure} end_node - The node we want to have a direct
	 *        path to.
	...
	/**
	 * runs the animation and restarts itself as long as the animation is not
	 * finished. After it is finished the observer listening for the finished
	 * event is notified. Don't use this method to start the animation!
	 */
	public void run() {
		...
		if (init) {
			...
			if (walking_path == null) {
				...
				Iterator<NodeFigure> nodesToBlock = Utility.getOptimizedRoute(
						map, start_node, total_end);
				...
				while (nodesToBlock.hasNext()) {
					...
					if (stateToBlock.geState() == State.UNBLOCKED) {
						...
						// unblock nodes again and clear animations of
						// trainfigure then call walktomethod again
						tf.stopAnimation();
						tf.clearAnimations();

						for (NodeFigure nodeToUnblock : nodesAlreadyBlocked) {
							State.statemap.get(nodeToUnblock.getName())
			...
			if (!walking_path.hasNext()) {
				...
				// this is needed, so that we can e.g. start a new animation
				...
				animationFigure
						.notifyAnimationListener(new AnimationFinishedEvent(
								animationFigure,
								AnimationFinishedEvent.MOVE_FINISHED));
				...
				Timetable tt = AnimationPlay.getP().getTimetable(
						animationFigure.getFigureId());
				if (!tt.isBlocked()) {
			...
			if (path != null) {
				...
				State.statemap.get(start_node.getName()).setState(
						State.UNBLOCKED);
		...
		try {
			int pathSpeed = getValueSpeed(animationFigure.getPath()
					.getPathSpeed());

			run_count += getMaxDrivingSpeed(pathSpeed, trainSpeed)
					+ AnimationControllerCanvas.getAnimationSpeed();
		...
		if (stop) {
			...
			// Stop can cause problems when we restart it later again and the
			// figure is not in a room and not
			// on a direct path: means it is between a path and a room...
			
			if (this.animationFigure.getNodeFigure() == null) {
				NodeFigure next_node = this.animationFigure
						.getDirection_to_node();
	...
	/**
	 * Starts the animation. Can also be used to restart the animation if it was
	 * stopped. To start(run) the animation only use this method!
	 */
	public void start() {
		this.stop = false;
		// notify Listeners
		animationFigure.notifyAnimationListener(new AnimationStartedEvent(
				animationFigure, AnimationStartedEvent.MOVE_STARTED));
		map.getDisplay().timerExec(0, this);
	}

	/**
	 * Stops the animation. The animation can be restarted with @see
	 * {@link #start()}
	 */
	public void stop() {
		this.stop = true;
