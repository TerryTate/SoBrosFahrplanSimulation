...
public class Utils {
	 ...
	 * @param {@link NodeFigure} start_node - The node is the starting point. We
	 *        look randomly for a path which leads away from this node.
	...
	/**
	 * 1. Extracts segments out of one path. We use a
	 * {@link ManhattanConnectionRouter} to route the path so the polyline which
	 * is drawn as path can have orthogonal lines (with start and end
	 * point).This is why all these lines must be separated into segments.
	 * 
	 * 2. If we have extracted the points, we calculate all points between all
	 * segments and add them as PointLists. We need this because we then can
	 * animate through every single point.
	 * 
	 * @param {@link PathFigure} path - The path from which we want to have the
	 *        segments and PointLists of each segment.
	 * @param {@link NodeFigure} end_node - The RoomFigure to which the path
	 *        leads. We need this because we need a direction.
	 * @param {@link Figure} mobile - the Figure we animate.
	 * @return
	 */

	public static PointList getSegments(PathFigure path, NodeFigure end_node,
			Figure mobile) {
